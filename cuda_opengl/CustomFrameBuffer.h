...
public:
    ...
    void bind() {
        ...
        glDrawBuffers(2, buffers); ///< TODO check if this needs
    }
    void unbind() {
...
public:
    ...
    GLuint create_depth_attachment(int image_width, int image_height) {
        ...
        glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_DEPTH_COMPONENT32, image_width, image_height);
...
private:
    GLuint create_framebuffer() {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, color_tex, 0 /*level*/);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, extra_tex, 0 /*level*/);
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, depth_rb);
        if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
            std::cerr << "Framebuffer not OK." << std::endl;
        glBindFramebufferEXT(GL_FRAMEBUFFER, 0); ///< avoid pollution
        return fbo;
    }
};
